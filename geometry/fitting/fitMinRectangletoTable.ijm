/* Returns a table with particle numbers and distance metrics.
 * Requires "Analyze Particles" to be run first.
 */
n = roiManager("count");
...
for (i=0; i<n; i++) {
	roiManager("select", i);
	dm = fitRectangle();
	setResult("Index", i, roiManager("index"));
	//setResult("i", i, i);
	setResult("Distance 1", i, dm[0]);
	setResult("Distance 2", i, dm[1]);
}
updateResults;
